  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.status === 'success') {
        ...
        setUnreadCount(data.data.enquiries.count + data.data.blog_activity.comment_count);
      }
    } catch (error) {
            ...
            <div key={idx}>
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  const isActive = pathname === item.href || (item.href === '/admin/settings' && pathname.startsWith('/admin/settings')) || (item.href === '/admin/reviews' && pathname.startsWith('/admin/reviews'))
                  return (
                    <Link
                      key={item.name}
                      href={item.href}
            ...
            <div className="hidden lg:flex items-center gap-1.5 mr-2">
              <div className="relative group/notif">
                <button
                  ...
                  className="p-2.5 text-slate-400 hover:text-primary transition-all relative"
                >
                  <Bell className="w-5 h-5" />
                  {unreadCount > 0 && (
                    <span className="absolute top-2 right-2.5 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full border-2 border-white flex items-center justify-center animate-bounce">
                  ...
                  <div className="absolute right-0 mt-2 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex flex-col max-h-[500px]">
                      ...
                      <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                        ...
                        <Link href="/admin/enquiries" className="text-[10px] font-bold text-primary hover:underline">View All</Link>
                      </div>
                      ...
                      <div className="overflow-y-auto">
                        {enquiries.length > 0 ? (
                          ...
                          <div className="p-6 text-center text-xs text-slate-400">No new enquiries</div>
                        )}
                      </div>

                      {/* Blog Activity Section */}
                      <div className="p-4 bg-slate-50 border-b border-slate-100 border-t flex items-center justify-between sticky top-0 z-10">
                        <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-2">
                          <Activity className="w-3 h-3 text-red-500" />
                          Blog Activity
                        </h3>
                        <Link href="/admin/blogs" className="text-[10px] font-bold text-primary hover:underline">View All</Link>
                      </div>
                        ...
                        {blogActivity.length > 0 ? (
                          blogActivity.map((activity, idx) => (
                            <Link
                              ...
                              className="flex items-start gap-3 p-4 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0"
                            >
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${activity.type === 'like' ? 'bg-red-100 text-red-500' : 'bg-green-100 text-green-600'}`}>
                          ...
                          <div className="p-6 text-center text-xs text-slate-400">No new blog activity</div>
                        )}
                      </div>
