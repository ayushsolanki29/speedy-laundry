    ...
    const fetchEnquiries = async () => {
        ...
        try {
            ...
            if (data.status === 'success') {
                // Filter only business-quote enquiries
                const businessEnquiries = data.data.filter(eq => eq.service === 'business-quote')
                setEnquiries(businessEnquiries)
            } else {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-display text-foreground">Business Enquiries</h1>
                    <p className="text-muted-foreground">Manage incoming quotes from commercial clients.</p>
                </div>
                <button
            ...
            <div className="space-y-4">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-border/60">
                        ...
                        <p className="text-muted-foreground font-bold text-lg">No business enquiries found</p>
