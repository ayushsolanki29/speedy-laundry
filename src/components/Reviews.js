          ...
          <div className="flex gap-4">
            {reviews.map((_, index) => (
              <button
                ...
                className={`group relative p-3 -m-3 focus:outline-none`}
                aria-label={`Go to review ${index + 1}`}
              >
                <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-primary w-8' : 'bg-gray-300 group-hover:bg-gray-400'
                  }`} />
              </button>
            ))}
          </div>
